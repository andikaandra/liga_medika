    ...
    {{-- <div class="container"> --}}
      <div class="col-md-12">
        <div class="alert alert-warning">
          <p>Hello <strong>{{Auth::user()->name}}</strong>. You have been assigned unique <strong>ID {{Auth::user()->id + 000}}</strong>. Registration fees are <strong>free</strong> until you reach the final stages
          <hr>
          <p>Literature Review & Research Public Poster wave: {{$lomba->gelombang_sekarang}}</p>
        </div>
          <form id="reset" method="post" action="{{route('reset.cabang')}}">
          @csrf
          <input type="hidden" name="user_id" value="{{Auth::user()->id}}">
          <div class="alert alert-info">
              After you click submit button you are officially choose Literature Review & Research Public Poster. If you would like to start over, you may <a href="#" onclick="$('#reset').submit(); return false;" id="submit">Click here to reset</a>.
          </div>
          </form>
        <div class="card">
