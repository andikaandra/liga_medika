    ...
    {{-- <div class="container"> --}}
      <div class="col-md-12">
        ...
        <div class="alert alert-warning">
          <p>Hello <strong>{{Auth::user()->name}}</strong>. You have been assigned unique <strong>ID {{Auth::user()->id + 000}}</strong>. The amount you must transfer to register IMSSO - Women Basketball is <strong>Rp {{ number_format($lomba->biaya + Auth::user()->id + 000 ,2,',','.')}}</strong>. This is to make sure the verification process is done fast.</p>
          <p>Rekening Pembayaran/ Bank Account for payment: <br> Nama: “REGISTRASI LIGA MEDIKA”, Bank
            Mandiri, 157-00-0476595-5
        </p>
          <hr>
          <p>IMSSO - Women Basketball wave: {{$lomba->gelombang_sekarang}}</p>
        </div>
        ...
        <form id="dataPeserta" method="post" enctype="multipart/form-data" action="{{route('register.imsso.women.basketball')}}">
        @csrf
        <div id="contentPanel">
