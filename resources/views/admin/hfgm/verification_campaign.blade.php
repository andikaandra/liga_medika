...
@section('script')
  <script type="text/javascript">
    $(document).ready(function(){
      ...
      let campaignTable = $(".table").DataTable({
        ajax: '{{url('admin/hfgm/campaign')}}',
        columns: [
          {data: 'id'},
          {data: 'nama'},
          {data: 'user.email'},
          ...
          {data: null,
            render: function(data, type, row) {
              if (row.status_verif == -1) {
                return "<button class='btn btn-success mr-2 accept' campaign-id='"+row.id+"'>Accept</button>"
                +"<button class='btn btn-danger mr-2 decline' campaign-id='"+row.id+"' disabled>Decline</button>" +
                "<button class='btn btn-info mr-2 info' campaign-id='"+row.id+"'>Info</button>";
              }
              else if (row.status_verif == 0) {
                return "<button class='btn btn-success mr-2 accept' campaign-id='"+row.id+"'>Accept</button>"
                +"<button class='btn btn-danger mr-2 decline' campaign-id='"+row.id+"'>Decline</button>" +
                "<button class='btn btn-info mr-2 info' campaign-id='"+row.id+"'>Info</button>";
              }
              else {
                return "<button class='btn btn-success mr-2 accept' campaign-id='"+row.id+"' disabled>Accept</button>"
                +"<button class='btn btn-danger mr-2 decline' campaign-id='"+row.id+"'>Decline</button>" +
                "<button class='btn btn-info mr-2 info' campaign-id='"+row.id+"'>Info</button>";
              }
            }
      ...
      $(document).on('click', '.info', async function(){
        const id = $(this).attr('campaign-id');
        let data;
        try {
          data = await $.ajax({
            url: '{{url('admin/hfgm/campaign')}}/' + id
          });
        } catch (e) {
          alert("Ajax error");
          console.log(e);
        ...
        console.log(data);
          ...
          path = '{{url("admin/view/hfgm/image/ktp")}}/' + data.campaign_data.id;
          $("#ktp").attr('href', path);
          $('.price').trigger('input');
      ...
      $(document).on('click', '.accept', function(){
        ...
        async function(){
          ...
          try {
            await $.ajax({
              url: '{{url('admin/hfgm/campaign/accept')}}/' + id,
              method: "PUT"
            });
          } catch (e) {
            alert("ajaax error");
      ...
      $(document).on('click', '.decline', function(){
        ...
        async function(){
          ...
          try {
            await $.ajax({
              url: '{{url('admin/hfgm/campaign/decline')}}/' + id,
              method: "PUT"
            });
          } catch (e) {
            alert("ajaax error");
