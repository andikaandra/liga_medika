...
@section('script')
  <script type="text/javascript">
    $(document).ready(function(){
      $("#admin-hfgm").addClass("active");
      $('.price').mask('0.000.000.000.000', {reverse: true});

      let concertTable = $(".table").DataTable({
        ajax: '{{url('admin/hfgm/concert')}}',
        columns: [
          {data: 'id'},
          {data: 'nama'},
          {data: 'user.email'},
          ...
          {data: null,
            render: function(data, type, row) {
              if (row.status_verif == -1) {
                return "<button class='btn btn-success mr-2 accept' concert-id='"+row.id+"'>Accept</button>"
                +"<button class='btn btn-danger mr-2 decline' concert-id='"+row.id+"' disabled>Decline</button>" +
                "<button class='btn btn-info mr-2 info' concert-id='"+row.id+"'>Info</button>";
              }
              else if (row.status_verif == 0) {
                return "<button class='btn btn-success mr-2 accept' concert-id='"+row.id+"'>Accept</button>"
                +"<button class='btn btn-danger mr-2 decline' concert-id='"+row.id+"'>Decline</button>" +
                "<button class='btn btn-info mr-2 info' concert-id='"+row.id+"'>Info</button>";
              }
              else {
                return "<button class='btn btn-success mr-2 accept' concert-id='"+row.id+"' disabled>Accept</button>"
                +"<button class='btn btn-danger mr-2 decline' concert-id='"+row.id+"'>Decline</button>" +
                "<button class='btn btn-info mr-2 info' concert-id='"+row.id+"'>Info</button>";
              }
            }
      ...
      $(document).on('click', '.info', async function(){
        const id = $(this).attr('concert-id');
        let data;
        try {
          data = await $.ajax({
            url: '{{url('admin/hfgm/concert')}}/' + id
          });
        } catch (e) {
          alert("Ajax error");
          console.log(e);
        ...
        console.log(data);
          let path = "{{url('admin/view/hfgm/image/payment').'/'}}";
          $(".modal-title").text("");
          ...
          path = '{{url("admin/view/hfgm/image/ktp")}}/' + data.concert_data.id;
          $("#ktp").attr('href', path);
          $('.price').trigger('input');
      ...
      $(document).on('click', '.accept', function(){
        ...
        async function(){
          ...
          try {
            await $.ajax({
              url: '{{url('admin/hfgm/concert/accept')}}/' + id,
              method: "PUT"
            });
          } catch (e) {
            alert("ajaax error");
          ...
          concertTable.ajax.reload(null, false);
        },
          function(){
      ...
      $(document).on('click', '.decline', function(){
        ...
        async function(){
          ...
          try {
            await $.ajax({
              url: '{{url('admin/hfgm/concert/decline')}}/' + id,
              method: "PUT"
            });
          } catch (e) {
            alert("ajaax error");
