  ...
  <script type="text/javascript">
    $(document).ready(function(){
      ...
      let dataTable = $(".table").DataTable({
        responsive: true,
        ajax: '{{url('admin/imsso/men-basketball')}}',
        columns: [
          {data: "id"},
          {data: "user.name"},
          {data: "user.email"},
          ...
          {data: null,
            render: function(data, type, row) {
              if (row.status_verif == -1) {
                return "<button class='btn btn-success mr-2 accept' imsso-id='"+row.id+"'>Accept</button>"
                +"<button class='btn btn-danger mr-2 decline' imsso-id='"+row.id+"' disabled>Decline</button>" +
                "<button class='btn btn-info mr-2 info' imsso-id='"+row.id+"'>Info</button>";
              }
              else if (row.status_verif == 0) {
                return "<button class='btn btn-success mr-2 accept' imsso-id='"+row.id+"'>Accept</button>"
                +"<button class='btn btn-danger mr-2 decline' imsso-id='"+row.id+"'>Decline</button>" +
                "<button class='btn btn-info mr-2 info' imsso-id='"+row.id+"'>Info</button>";
              }
              else {
                return "<button class='btn btn-success mr-2 accept' imsso-id='"+row.id+"' disabled>Accept</button>"
                +"<button class='btn btn-danger mr-2 decline' imsso-id='"+row.id+"'>Decline</button>" +
                "<button class='btn btn-info mr-2 info' imsso-id='"+row.id+"'>Info</button>";
              }
            }
      ...
      $(document).on('click', '.info', async function(){
        const id = $(this).attr('imsso-id');
        let data;

        try {
            data = await $.ajax({
              url: '{{url('admin/imsso/imsso')}}/' + id
            });
        } catch (e) {
          alert("Ajax error");
          console.log(e);
        ...
        path = '{{url('admin/imsso/file')}}/' + data.user_id; //path for participant files
      ...
      $(document).on('click', '.accept', function(){
        ...
        async function(){
          ...
          try {
            await $.ajax({
              url: '{{url('admin/imsso/imsso/accept')}}/' + id,
              method: "PUT"
            });
          } catch (e) {
            alert("ajaax error");
      ...
      $(document).on('click', '.decline', function(){
        ...
        async function(){
          ...
          try {
            await $.ajax({
              url: '{{url('admin/imsso/imsso/decline')}}/' + id,
              method: "PUT"
            });
          } catch (e) {
            alert("ajaax error");
